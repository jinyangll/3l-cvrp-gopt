...
void LoadingChecker::init_python_interpreter() {
  if (!guard) {
...
std::vector<Cuboid> LoadingChecker::SelectItems(
    const Collections::IdVector& nodeIds, std::vector<Group>& nodes,
...
LoadingStatus LoadingChecker::PackingHeuristic(
    PackingType packingType, const Container& container,
...
LoadingStatus LoadingChecker::ConstraintProgrammingSolver(
    PackingType packingType, const Container& container,
    const boost::dynamic_bitset<>& set, const Collections::IdVector& stopIds,
    const std::vector<Cuboid>& items, bool isCallTypeExact, double maxRuntime) {
  if (maxRuntime < 0.0 + 1e-5) {
...
LoadingStatus LoadingChecker::HeuristicCompleteCheck(
...
void LoadingChecker::SetBinPackingModel(GRBEnv* env,
  ...
  mBinPacking1D =
      std::make_unique<BinPacking1D>(env, containers, nodes, outputPath);
}

LoadingStatus LoadingChecker::RunLoadingHeuristic(
    PackingType packingType [[maybe_unused]],
...
int LoadingChecker::SolveBinPackingApproximation() const {
  return mBinPacking1D->Solve();
}

int LoadingChecker::ReSolveBinPackingApproximation(
    const boost::dynamic_bitset<>& selectedGroups) const {
...
int LoadingChecker::DetermineMinVehicles(bool enableLifting,
                                         ...
                                         double weight, double volume) const {
  auto weightRatio =
      container.WeightLimit > 1 ? weight / container.WeightLimit : 0.0;
  ...
  int minVehicles = DetermineMinVehiclesBinPacking(
      enableLifting, liftingThreshold, nodes, r, z);

  return minVehicles;
}

int LoadingChecker::DetermineMinVehiclesBinPacking(
    bool enableLifting, double liftingThreshold,
    const boost::dynamic_bitset<>& nodes, int r, double z) const {
...
void LoadingChecker::AddFeasibleSequenceFromOutside(
...
bool LoadingChecker::RouteIsInFeasSequences(
  ...
  return mFeasSequences.at(Parameters.LoadingProblem.LoadingFlags)
      .contains(route);
}

void LoadingChecker::AddSequenceCheckedTwoOpt(
    const Collections::IdVector& sequence) {
  mTwoOptCheckedSequences.insert(sequence);
}

bool LoadingChecker::SequenceIsCheckedTwoOpt(
    const Collections::IdVector& sequence) const {
  return mTwoOptCheckedSequences.contains(sequence);
}

boost::dynamic_bitset<> LoadingChecker::MakeBitset(
...
bool LoadingChecker::SequenceIsInfeasibleCP(
    const Collections::IdVector& sequence, const LoadingFlag mask) const {
...
bool LoadingChecker::SetIsInfeasibleCP(const boost::dynamic_bitset<>& set,
  ...
  if (!IsSet(mask, LoadingFlag::Support)) {
    ...
    if (std::find_if(std::begin(sets), std::end(sets), setComparer) !=
        std::end(sets)) {
      return true;
    }
  } else {
...
bool LoadingChecker::SetIsFeasibleCP(const boost::dynamic_bitset<>& set,
  ...
  if (!IsSet(mask, LoadingFlag::Support)) {
    // If support is disabled, set S is feasible when S is a subset of a
    // feasible set.
    ...
    // removing items can lead to infeasibility.
...
LoadingFlag LoadingChecker::BuildMask(PackingType type) const {
  switch (type) {
    ...
    case PackingType::LifoNoSequence:
      return LoadingFlag::LifoNoSequence &
             Parameters.LoadingProblem.LoadingFlags;
    default:
...
LoadingStatus LoadingChecker::GetPrecheckStatusCP(
    const Collections::IdVector& sequence, const boost::dynamic_bitset<>& set,
    const LoadingFlag mask, const bool isCallTypeExact) {
...
void LoadingChecker::AddStatus(const Collections::IdVector& sequence,
  ...
  // Add to feasible sequences although lifo might be disabled; needed for SP
  // heuristic.
  if (status == LoadingStatus::FeasOpt &&
      mask == Parameters.LoadingProblem.LoadingFlags) {
    AddFeasibleRoute(sequence);
