...
void BranchAndCutSolver::Initialize() {
  mLogFile << "ProblemVariant: "
           << (int)mInputParameters.ContainerLoading.LoadingProblem.Variant
           << "\n";
  ...
  for (const auto& node : mInstance->GetCustomers()) {
    customerNodes.emplace_back(node.InternId, node.ExternId, node.PositionX,
                               node.PositionY, node.TotalWeight,
                               node.TotalVolume, node.TotalArea, node.Items);
  }

  mLoadingChecker =
      std::make_unique<LoadingChecker>(mInputParameters.ContainerLoading);
  mLoadingChecker->SetBinPackingModel(mEnv, containers, customerNodes,
                                      mOutputPath);
  ...
  for (const auto& customer : mInstance->GetCustomers()) {
    ...
    auto items =
        InterfaceConversions::SelectItems(route, mInstance->Nodes, false);

    auto heurStatus = mLoadingChecker->PackingHeuristic(
        PackingType::Complete, containers[0], route, items);
    ...
    auto exactStatus = mLoadingChecker->ConstraintProgrammingSolverPython(
        ...
        items, mInputParameters.IsExact(BranchAndCutParams::CallType::Exact));
...
void BranchAndCutSolver::TestProcedure() {
  LoadingFlag mask = LoadingFlag::NoOverlap | LoadingFlag::Fragility |
                     LoadingFlag::Lifo | LoadingFlag::Support;
...
void BranchAndCutSolver::StartSolutionProcedure() {
  ...
  for (const auto& route : startRoutes) {
    mStartSolutionArcs.emplace_back(1.0, mInstance->GetDepotId(),
                                    route.Sequence.front());
    ...
    mStartSolutionArcs.emplace_back(1.0, route.Sequence.back(),
                                    mInstance->GetDepotId());
  }

  for (size_t k = 0; k < startRoutes.size(); ++k) {
    const auto& route = startRoutes[k];

    mStartSolution.Costs +=
        Evaluator::CalculateRouteCosts(mInstance, route.Sequence);
    ...
    mStartSolution.Tours.emplace_back(mInstance->Nodes[mInstance->DepotIndex],
                                      vehicle, std::move(sequence));
  }
  ...
  mLogFile << "Start solution with " << mStartSolution.NumberOfRoutes
           << " Vehicles and total costs " << mStartSolution.Costs << " in "
           << mTimer.StartSolution.count() << " s.\n";
...
std::vector<Route> BranchAndCutSolver::GenerateStartSolution() {
  auto startSolution =
      Heuristics::Constructive::ModifiedSavings(mInstance, &mInputParameters,
                                                mLoadingChecker.get(), &mRNG)
          .Run();
  ...
  for (const auto& route : mLoadingChecker->GetFeasibleRoutes()) {
    ...
    LocalSearch::RunIntraImprovement(mInstance, mLoadingChecker.get(),
                                     &mInputParameters, route);
  }

  auto spHeuristic = Heuristics::SetBased::SPHeuristic(
      mInstance, mLoadingChecker.get(), &mInputParameters, mEnv);
...
std::vector<Route> BranchAndCutSolver::SetGivenStartSolution() {
  ...
  std::string problemVariantString =
      mInputParameters.ContainerLoading.LoadingProblem.GetVariantString();
  std::string startSolutionFilePath = mStartSolutionFolderPath +
                                      problemVariantString + "/" +
                                      mInstance->Name + ".json";

  bool isRegularFile = std::filesystem::is_regular_file(startSolutionFilePath);
  if (!isRegularFile) {
    std::string exceptionMessage =
        startSolutionFilePath + " is not a regular file.";
  ...
  for (size_t id = 0; id < startSolution.size(); id++) {
    ...
    auto tour =
        Tour(mInstance->Nodes[0], mInstance->Vehicles[id], nodesInRoute);

    if (totalWeight > container.WeightLimit) {
      throw std::runtime_error(
          "Route " + std::to_string(id) + tour.Print() + " with total weight " +
    ...
    if (totalVolume > container.Volume) {
      throw std::runtime_error(
          "Route " + std::to_string(id) + tour.Print() + " with total volume " +
          std::to_string(totalVolume) + " exceeds volume limit " +
          std::to_string(container.Volume));
    }

    logFile << "Route " << std::to_string(id) + tour.Print() << ": nItems "
            << std::to_string(selectedItems.size())
            << " | weight util " +
                   std::to_string(totalWeight / container.WeightLimit)
            << " | volume util " +
                   std::to_string(totalVolume / container.Volume)
            << " | ";

    if (!mInputParameters.ContainerLoading.LoadingProblem
             .EnableThreeDimensionalLoading) {
      logFile << "\n";
      continue;
    }

    clock.start();
    auto heuristicStatus = mLoadingChecker->PackingHeuristic(
        PackingType::Complete, container, sequence, selectedItems);
    clock.end();
    ...
    std::string feasStatusHeur =
        heuristicStatus == LoadingStatus::FeasOpt ? "feasible" : "infeasible";
    logFile << feasStatusHeur
            << " with packing heuristic : " << std::to_string(clock.elapsed())
            << " | ";
    ...
    std::string feasStatusCP =
        exactStatus == LoadingStatus::FeasOpt ? "feasible" : "infeasible";
    logFile << feasStatusCP
            << " with CP model : " << std::to_string(clock.elapsed()) << "\n";
...
std::vector<Route> BranchAndCutSolver::SetHardCodedStartSolution() {
  ...
  std::vector<Collections::IdVector> startSequences = {
      Collections::IdVector{4, 13, 14}, Collections::IdVector{5, 10, 15, 12},
      Collections::IdVector{11, 9, 2, 1}, Collections::IdVector{3, 8, 7, 6}};
  ...
  for (size_t id = 0; id < startSolution.size(); id++) {
    ...
    mLogFile << "Route " << std::to_string(id) + tour.Print() << ": nItems "
             << std::to_string(selectedItems.size())
             << " | weight util " +
                    std::to_string(totalWeight / container.WeightLimit)
             << " | volume util " +
...
void BranchAndCutSolver::InfeasibleArcProcedure() {
  ...
  mLogFile << "Deleted arcs: " << std::to_string(mInfeasibleArcs.size())
           << "\n";
  mLogFile << "Infeasible tail paths: "
           << std::to_string(mInfeasibleTailPaths.size()) << "\n";
  mLogFile << "Infeasible 2-node combinations: "
           << std::to_string(mLoadingChecker->GetSizeInfeasibleCombinations())
           << "\n";
  ...
  // mLogFile << "Infeasible customer combinations size 3: " <<
  // std::to_string(mInfeasibleCombinations.size()) <<
  // "\n";
}
...
void BranchAndCutSolver::DetermineInfeasiblePaths() {
  ...
  for (size_t iNode = 1; iNode < nodes.size() - 1; ++iNode) {
    ...
    for (size_t jNode = iNode + 1; jNode < nodes.size(); ++jNode) {
      if (nodes[iNode].TotalWeight + nodes[jNode].TotalWeight >
              container.WeightLimit ||
          nodes[iNode].TotalVolume + nodes[jNode].TotalVolume >
              container.Volume) {
      ...
      auto selectedItems = InterfaceConversions::SelectItems(
          selectedNodes, mInstance->Nodes, false);
      bool forwardRelaxedInfeasible =
          !CheckPath(selectedNodes, container, selectedItems);

      std::swap(selectedNodes[0], selectedNodes[1]);
      selectedItems = InterfaceConversions::SelectItems(
          selectedNodes, mInstance->Nodes, false);
      bool backwardRelaxedInfeasible =
          !CheckPath(selectedNodes, container, selectedItems);
...
bool BranchAndCutSolver::CheckPath(const Collections::IdVector& path,
                                   Container& container,
                                   std::vector<Cuboid>& items) {
  if (mInputParameters.BranchAndCut.ActivateHeuristic) {
    auto heuristicStatus = mLoadingChecker->PackingHeuristic(
        PackingType::Complete, container, path, items);
    if (heuristicStatus == LoadingStatus::FeasOpt) {
      return true;
    }
  }
  ...
  auto statusSupportRelaxation =
      mLoadingChecker->ConstraintProgrammingSolverPython(
          ...
          mLoadingChecker->MakeBitset(mInstance->Nodes.size(), path), path,
          items, mInputParameters.IsExact(BranchAndCutParams::CallType::Exact));
...
void BranchAndCutSolver::DetermineExtendedInfeasiblePath() {
  ...
  for (auto& arc : mInfeasibleTailPaths) {
    ...
    for (const auto& nodeK : mInstance->GetCustomers()) {
      if (nodeK.InternId == nodeI.InternId ||
          nodeK.InternId == nodeJ.InternId) {
        continue;
      }

      if (weight + nodeK.TotalWeight > container.WeightLimit ||
          volume + nodeK.TotalVolume > container.Volume) {
        continue;
      }

      Collections::IdVector path = {nodeI.InternId, nodeJ.InternId,
                                    nodeK.InternId};

      auto selectedItems =
          InterfaceConversions::SelectItems(path, mInstance->Nodes, false);

      auto heuristicStatus = LoadingStatus::Infeasible;

      if (mInputParameters.BranchAndCut.ActivateHeuristic) {
        heuristicStatus = mLoadingChecker->PackingHeuristic(
  ...
  for (size_t iNode = 1; iNode < nodes.size() - 2; ++iNode) {
    for (size_t jNode = iNode + 1; jNode < nodes.size() - 1; ++jNode) {
      for (size_t kNode = jNode + 1; kNode < nodes.size(); ++kNode) {
        double totalWeight = nodes[iNode].TotalWeight +
                             nodes[jNode].TotalWeight +
                             nodes[kNode].TotalWeight;
        double totalVolume = nodes[iNode].TotalVolume +
                             nodes[jNode].TotalVolume +
                             nodes[kNode].TotalVolume;
        ...
        if (heuristicStatus == LoadingStatus::Infeasible) {
          ...
          auto status = mLoadingChecker->ConstraintProgrammingSolverPython(
              PackingType::NoSupportNoSequence, container, nodesInSet, path,
              selectedItems,
              mInputParameters.IsExact(BranchAndCutParams::CallType::Exact),
              maxRuntime);
...
size_t BranchAndCutSolver::DetermineLowerBoundVehicles() {
  ...
  auto lowerBound1D =
      static_cast<size_t>(mLoadingChecker->SolveBinPackingApproximation());
...
void BranchAndCutSolver::Solve() {
  ...
  branchAndCut.BuildModel(mStartSolutionArcs, mInfeasibleArcs,
                          mInfeasibleTailPaths);
  auto callback = CallbackFactory::CreateCallback(
      mInputParameters.ContainerLoading.LoadingProblem.Variant, mEnv,
      *branchAndCut.GetXVariables(), mInstance, mLoadingChecker.get(),
      &mInputParameters, mOutputPath);
  ...
  auto statistics = SolverStatistics(
      ...
      branchAndCut.GetNodeCount(), branchAndCut.GetSimIterCount(),
      callback->CallbackTracker, mTimer, mInfeasibleArcs.size(),
      mInfeasibleTailPaths.size());

  std::string solutionStatisticsString =
      "SolutionStatistics-" + mInstance->Name;
  Serializer::WriteToJson(statistics, mOutputPath, solutionStatisticsString);

  if (const auto optionalSolution = branchAndCut.GetSolution();
      optionalSolution.has_value()) {
...
void BranchAndCutSolver::DeterminePackingSolution() {
  ...
  for (size_t tourId = 0; tourId < mFinalSolution.Tours.size(); tourId++) {
    ...
    if (totalWeight > container.WeightLimit) {
      throw std::runtime_error(
          "Route " + std::to_string(tourId) + tour.Print() +
          " with total weight " + std::to_string(totalWeight) +
    ...
    if (totalVolume > container.Volume) {
      throw std::runtime_error(
          "Route " + std::to_string(tourId) + tour.Print() +
          " with total volume " + std::to_string(totalVolume) +
    ...
    if (mInputParameters.BranchAndCut.ActivateHeuristic) {
      heuristicStatus = mLoadingChecker->PackingHeuristic(
          PackingType::Complete, container, stopIds, selectedItems);
    }
...
void BranchAndCutSolver::WriteSolutionSolutionValidator() {
  ResultWriter::SolutionValidator::WriteInput(
      mOutputPath, mInstance->Name,
      InterfaceConversions::NodesToGroup(mInstance->Nodes),
      mInstance->Vehicles[0].Containers[0], mInstance->Vehicles.size());
  ...
  ResultWriter::SolutionValidator::WriteOutput(mOutputPath, mInstance->Name,
                                               routes, mFinalSolution.Costs);
}
