...
LazyConstraintsGenerator::TwoPathInequalityLifting(
    const Collections::IdVector& sequence, const boost::dynamic_bitset<>& set,
...
std::optional<std::vector<Cut>> LazyConstraintsGenerator::RegularPathLifting(
...
LazyConstraintsGenerator::DetermineMinimalInfeasibleSubset(
    const Collections::IdVector& sequence, boost::dynamic_bitset<>& set,
    const Container& container) {
  std::vector<std::pair<double, size_t>> volumes;
  for (const auto nodeI : sequence) {
    volumes.push_back(
        std::make_pair(mInstance->Nodes[nodeI].TotalVolume, nodeI));
  }
  ...
  while (indexLastNode < sortedSubset.size()) {
    auto subSet = std::vector(
        std::begin(sortedSubset) + static_cast<long>(indexLastNode) + 1,
        std::end(sortedSubset));
  ...
  return std::vector(
      std::begin(sortedSubset) + static_cast<long>(indexLastNode),
      std::end(sortedSubset));
}

std::optional<Collections::IdVector>
LazyConstraintsGenerator::DetermineMinimalInfeasibleSubPath(
    const Collections::IdVector& sequence, const Container& container,
    bool fromFront) {
  size_t indexLastNode = 0;
  ...
  while (indexLastNode < sequence.size()) {
    Collections::IdVector subPath =
        fromFront
            ? std::vector(
                  std::begin(sequence) + static_cast<long>(indexLastNode) + 1,
                  std::end(sequence))
            : std::vector(
                  std::begin(sequence),
                  std::end(sequence) - 1 - static_cast<long>(indexLastNode));
    ...
    auto items =
        InterfaceConversions::SelectItems(subPath, mInstance->Nodes, false);

    auto heuristicStatus = mLoadingChecker->PackingHeuristic(
        PackingType::Complete, container, subPath, items);

    if (heuristicStatus == LoadingStatus::FeasOpt) {
      LocalSearch::RunIntraImprovement(mInstance, mLoadingChecker,
                                       mInputParameters, subPath);
      break;
    }
    ...
    if (!mInputParameters->ContainerLoading.LoadingProblem.EnableSupport &&
        cpStatus == LoadingStatus::FeasOpt) {
      LocalSearch::RunIntraImprovement(mInstance, mLoadingChecker,
                                       mInputParameters, subPath);
    }
  ...
  return fromFront ? std::vector(std::begin(sequence) +
                                     static_cast<long>(indexLastNode),
                                 std::end(sequence))
                   : std::vector(
                         std::begin(sequence),
                         std::end(sequence) - static_cast<long>(indexLastNode));
}

std::vector<Cut> LazyConstraintsGenerator::CreateTwoPathCuts(
    const Collections::IdVector& sequence, const boost::dynamic_bitset<>& set,
    const Container& container) {
  auto tmpSet = set;
...
std::vector<Cut> LazyConstraintsGenerator::CreateRegularPathCuts(
    const Collections::IdVector& sequence, Container& container) {
  std::vector<Cut> cuts;
  cuts.reserve(3);

  auto minimalPathFromFront =
      DetermineMinimalInfeasibleSubPath(sequence, container, true);
  if (minimalPathFromFront.has_value()) {
    cuts.emplace_back(CreateConstraint(CutType::RegularPathFront,
                                       minimalPathFromFront.value()));
  }

  auto minimalPathFromBack =
      DetermineMinimalInfeasibleSubPath(sequence, container, false);
  if (minimalPathFromBack.has_value() &&
      (minimalPathFromFront != minimalPathFromBack)) {
    cuts.emplace_back(CreateConstraint(CutType::RegularPathBack,
                                       minimalPathFromBack.value()));
  }
...
Cut LazyConstraintsGenerator::CreateConstraint(
    CutType type, const Collections::IdVector& sequence,
  ...
  switch (type) {
    ...
    case CutType::SEC:
      ...
      return CreateSubtourEliminationConstraint(type, sequence,
                                                minNumberVehicles);
...
Cut LazyConstraintsGenerator::CreateSubtourEliminationConstraint(
    CutType type, const Collections::IdVector& sequence,
    int minNumberVehicles) {
  /*
...
Cut LazyConstraintsGenerator::CreateTwoPathTailConstraint(
    CutType type, const Collections::IdVector& sequence) {
  auto cut = Cut(type);
...
Cut LazyConstraintsGenerator::CreateTournamentConstraint(
    CutType type, const Collections::IdVector& sequence) {
  auto cut = Cut(type);
  for (size_t i = 0; i < sequence.size() - 1; ++i) {
    const auto nodeI = sequence[i];
...
Cut LazyConstraintsGenerator::CreateTailTournamentConstraint(
    CutType type, const Collections::IdVector& sequence) {
  /*
  std::vector<int> sequenceCopy;
  for (int node : sequence)
...
Cut LazyConstraintsGenerator::CreateUndirectedInfeasiblePathConstraint(
    CutType type, const Collections::IdVector& sequence) {
  auto cut = Cut(type);
...
Cut LazyConstraintsGenerator::CreateUndirectedInfeasibleTailPathConstraint(
    CutType type, const Collections::IdVector& sequence) {
  auto cut = Cut(type);
...
Cut LazyConstraintsGenerator::CreateInfeasibleTailPathConstraint(
    CutType type, const Collections::IdVector& sequence) {
  auto cut = Cut(type);
