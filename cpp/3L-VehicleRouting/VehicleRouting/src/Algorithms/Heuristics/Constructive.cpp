...
std::vector<Route> Savings::Run() {
  ...
  for (const auto& nodeI : mInstance->GetCustomers()) {
    for (const auto& nodeJ : mInstance->GetCustomers()) {
      ...
      const auto savings = Evaluator::CalculateSavings(
          mInstance, nodeI.InternId, nodeJ.InternId);
  ...
  while (!savingsValues.empty()) {
    ...
    if (frontRoute.TotalVolume + backRoute.TotalVolume <= container.Volume &&
      ...
      auto frontSequence =
          frontRoute.Sequence;  // intended copy as we modify the sequence

      bool feasiblePackingFound =
          ConcatRoutes(frontSequence, backRoute.Sequence, container);
      ...
      if (feasiblePackingFound) {
        ...
        std::erase_if(savingsValues, [&](const auto& savingsTuple) {
    ...
    std::erase_if(savingsValues, [&](const auto& savingsTuple) {
      return std::get<1>(savingsTuple) == nodeI &&
             std::get<2>(savingsTuple) == nodeJ;
    });
  }
...
bool Savings::ConcatRoutes(Collections::IdVector& frontSequence,
  ...
  frontSequence.insert(std::end(frontSequence), std::begin(backSequence),
                       std::end(backSequence));

  if (mInputParameters->ContainerLoading.LoadingProblem.LoadingFlags ==
      LoadingFlag::NoneSet) {
  ...
  auto selectedItems =
      InterfaceConversions::SelectItems(frontSequence, mInstance->Nodes, false);

  double maxRuntime = mInputParameters->DetermineMaxRuntime(
      BranchAndCutParams::CallType::Heuristic);

  /////////////////////////////
  auto status = mLoadingChecker->HeuristicCompleteCheckForTest(
      container,
      mLoadingChecker->MakeBitset(mInstance->Nodes.size(), frontSequence),
      frontSequence, selectedItems, maxRuntime);

  return status == LoadingStatus::FeasOpt;
}

void Savings::DeleteSavings(
    std::vector<std::tuple<double, size_t, size_t>>& savingsValues,
    size_t startNode, size_t endNode) {
  std::erase_if(savingsValues, [&](const auto& savingsTuple) {
    return std::get<1>(savingsTuple) == startNode ||
           std::get<2>(savingsTuple) == endNode ||
           (std::get<1>(savingsTuple) == endNode &&
            std::get<2>(savingsTuple) == startNode);
  });
}

std::vector<Route> ModifiedSavings::Run() {
  auto startSolution =
      Savings(mInstance, mInputParameters, mLoadingChecker).Run();
...
void ModifiedSavings::RepairProcedure(std::vector<Route>& solution) {
  std::ranges::sort(solution, [](const auto& routeA, const auto& routeB) {
    return routeA.TotalVolume > routeB.TotalVolume;
  });
  ...
  solution.erase(
      std::begin(solution) + static_cast<int>(mInstance->Vehicles.size()),
      std::end(solution));

  while (!notVisitedCustomers.empty()) {
    std::ranges::sort(notVisitedCustomers,
                      [&](const auto& idA, const auto& idB) {
                        return mInstance->Nodes[idA].TotalVolume <
                               mInstance->Nodes[idB].TotalVolume;
                      });

    auto nodeToInsert = notVisitedCustomers.back();

    auto insertionCosts =
        DetermineInsertionCostsAllRoutes(solution, nodeToInsert);
    ...
    if (!nodeInserted) {
      ...
      notVisitedCustomers.insert(std::end(notVisitedCustomers),
                                 std::begin(removedNodes),
                                 std::end(removedNodes));
    }
  }
}

std::vector<std::tuple<double, size_t, size_t>>
ModifiedSavings::DetermineInsertionCostsAllRoutes(std::vector<Route>& solution,
                                                  size_t nodeId) {
  const auto& container = mInstance->Vehicles[0].Containers[0];
  ...
  for (size_t k = 0; k < solution.size(); ++k) {
    ...
    if (route.TotalVolume + node.TotalVolume <= container.Volume &&
      ...
      insertionCosts.insert(std::end(insertionCosts),
                            std::begin(newInsertionCosts),
                            std::end(newInsertionCosts));
    }
  }
...
std::vector<std::tuple<double, size_t, size_t>>
ModifiedSavings::DetermineInsertionCosts(const Route& route, size_t routeId,
                                         size_t nodeId) {
  std::vector<std::tuple<double, size_t, size_t>> insertionCosts;
  ...
  auto deltaCosts = Evaluator::CalculateInsertionCosts(
      mInstance, depotId, route.Sequence.front(), nodeId);

  insertionCosts.emplace_back(deltaCosts, routeId, depotId);

  for (size_t insertionPosition = 0;
       insertionPosition < route.Sequence.size() - 1; ++insertionPosition) {
    auto nodeA = route.Sequence[insertionPosition];
    auto nodeB = route.Sequence[insertionPosition + 1];
    deltaCosts =
        Evaluator::CalculateInsertionCosts(mInstance, nodeA, nodeB, nodeId);

    insertionCosts.emplace_back(deltaCosts, routeId, insertionPosition + 1);
  }

  deltaCosts = Evaluator::CalculateInsertionCosts(
      mInstance, route.Sequence.back(), depotId, nodeId);
...
Collections::IdVector ModifiedSavings::InsertInRandomRoute(
    Route& route, size_t nodeToInsert) {
  const auto& container = mInstance->Vehicles[0].Containers[0];
  ...
  while (!nodeInserted) {
    std::uniform_int_distribution<> distrib(
        0, static_cast<int>(route.Sequence.size() - 1));
...
bool ModifiedSavings::InsertionFeasible(Route& route, size_t nodeToInsert,
                                        size_t position) {
  auto container = mInstance->Vehicles[0].Containers[0];
  auto tmpSequence = route.Sequence;

  tmpSequence.insert(std::begin(tmpSequence) + static_cast<int>(position),
                     nodeToInsert);
  ...
  auto selectedItems =
      InterfaceConversions::SelectItems(tmpSequence, mInstance->Nodes, false);
  ...
  auto status = mLoadingChecker->HeuristicCompleteCheckForTest(
      ...
      tmpSequence, selectedItems, maxRuntime);
