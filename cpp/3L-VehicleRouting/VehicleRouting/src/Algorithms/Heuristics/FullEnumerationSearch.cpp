...
void FullEnumerationSearch::Run(const Instance* const instance,
  ...
  for (auto& move : moves) {
    ...
    if (!loadingChecker->Parameters.LoadingProblem.EnableLifo &&
        loadingChecker->RouteIsInFeasSequences(newRoute)) {
      loadingChecker->AddFeasibleSequenceFromOutside(sequence);
      break;
    }

    auto selectedItems =
        InterfaceConversions::SelectItems(sequence, instance->Nodes, false);

    const auto& container = instance->Vehicles.front().Containers.front();

    double maxRuntime = inputParameters.DetermineMaxRuntime(
        BranchAndCutParams::CallType::Heuristic);
    //////////////////
    auto status = loadingChecker->HeuristicCompleteCheckForTest(
        container, set, sequence, selectedItems, maxRuntime);
