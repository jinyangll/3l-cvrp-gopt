...
std::optional<Collections::SequenceVector> SPHeuristic::Run(double cutoff) {
  ...
  auto setCovering = BaseModels::SetCovering<Collections::SequenceVector>(
      mEnv, true, mLoadingChecker->GetFeasibleRoutes(), mInstance->CustomerIds,
      costs, mInstance->Vehicles.size());
  ...
  auto setPartitioning =
      BaseModels::SetPartitioning<Collections::SequenceVector>(
          mEnv, false, mLoadingChecker->GetFeasibleRoutes(),
          mInstance->CustomerIds, costs, mInstance->Vehicles.size());
...
void SPHeuristic::AddNewRoutes(auto& routes) {
  ...
  for (const auto& route : routes) {
    ...
    ////////////
    const auto status = mLoadingChecker->HeuristicCompleteCheckForTest(
        container, mLoadingChecker->MakeBitset(mInstance->Nodes.size(), route),
        route, items, maxRuntime);

    if (status == LoadingStatus::Invalid) {
      return;
    }

    // Always check new route with IntraImprovement, even though it is
    // infeasible
    LocalSearch::RunIntraImprovement(mInstance, mLoadingChecker,
                                     mInputParameters, route);
  }
...
Collections::SequenceVector SPHeuristic::CreateRoutesCustomerRemoval(
    auto& routes) {
