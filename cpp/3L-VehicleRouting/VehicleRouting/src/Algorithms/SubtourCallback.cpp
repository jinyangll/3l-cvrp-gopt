    ...
    switch (where) {
      case GRB_CB_MIP: {
        ...
        CallbackTracker.UpdateLowerBound(this->getDoubleInfo(GRB_CB_RUNTIME),
                                         this->getDoubleInfo(GRB_CB_MIP_NODCNT),
                                         objBound);
      } break;
      case GRB_CB_MIPNODE: {
        mCurrentNode = static_cast<unsigned int>(
        ...
        CallbackTracker.UpdateLowerBound(
            this->getDoubleInfo(GRB_CB_RUNTIME),
            this->getDoubleInfo(GRB_CB_MIPNODE_NODCNT),
            this->getDoubleInfo(GRB_CB_MIPNODE_OBJBND));

        // Check fractional solution
        auto checkFractionalSolutionTime = measure<>::duration(
            std::bind_front(&SubtourCallback::CheckFractionalSolution, this));
        CallbackTracker.UpdateElement(
            CallbackElement::FractionalSolutions,
            static_cast<uint64_t>(checkFractionalSolutionTime.count()));

        // Solve set partitioning heuristic
        auto [spHeuristicCalled, heuristicTime] =
            measure<>::durationWithReturn(std::bind_front(
                &SubtourCallback::SolveSetPartitioningHeuristic, this));
        if (!spHeuristicCalled) {
          break;
        }

        CallbackTracker.UpdateElement(
            CallbackElement::SPHeuristic,
            static_cast<uint64_t>(heuristicTime.count()));
      } break;
      case GRB_CB_MIPSOL: {
        mCurrentNode = static_cast<unsigned int>(
            this->getDoubleInfo(GRB_CB_MIPSOL_NODCNT));
        ////mLogFile << mCurrentNode << " | SOL | Feas Routes: " <<
        ////mLoadingChecker->GetFeasibleRoutes().size()<< "\n";

        // Check integer solution
        auto checkIntegerSolutionTime = measure<>::duration(
            std::bind_front(&SubtourCallback::CheckIntegerSolution, this));
        CallbackTracker.UpdateElement(
            CallbackElement::IntegerSolutions,
...
void SubtourCallback::InitializeCuts() {
  ...
  GraphFunctions::SetValues(1, nCustomerNodes, nNodes,
                            mInputParameters->UserCut.EpsForIntegrality);
  ...
  if (weightLimit > 0) {
    mCutTypesFractional.emplace_back(std::make_shared<RCCut>(
        mInputParameters, nCustomerNodes, weightLimit, weight, mInstance,
        &mCVRPSEPGraph, mLoadingChecker));
  }

  if (volumeLimit > 0) {
    mCutTypesFractional.emplace_back(std::make_shared<RCCut>(
        mInputParameters, nCustomerNodes, volumeLimit, volume, mInstance,
        &mCVRPSEPGraph, mLoadingChecker));
  }
  ...
  mCutTypesFractional.emplace_back(
      std::make_shared<MSTAR>(mInputParameters, nCustomerNodes, weightLimit,
                              weight, mInstance, &mCVRPSEPGraph));
  // // mCutTypesFractional.emplace_back(
  // //     std::make_shared<MSTAR>(mInputParameters, nCustomerNodes,
  // volumeLimit, volume, mInstance,
  // // &mCVRPSEPGraph));
  ...
  mCutTypesFractional.emplace_back(
      std::make_shared<SCI>(mInputParameters, nCustomerNodes, weightLimit,
                            weight, mInstance, &mCVRPSEPGraph));

  mCutTypesFractional.emplace_back(
      std::make_shared<GLM>(mInputParameters, nCustomerNodes, weightLimit,
                            weight, mInstance, &mCVRPSEPGraph));
  // // mCutTypesFractional.emplace_back(
  // //     std::make_shared<GLM>(mInputParameters, nCustomerNodes, volumeLimit,
  // volume, mInstance, &mCVRPSEPGraph));
}
...
void SubtourCallback::FillXVarValuesNode() {
  for (size_t iNode = 0, n = mInstance->Nodes.size(); iNode < n - 1; ++iNode) {
    for (size_t jNode = iNode + 1; jNode < n; ++jNode) {
      ...
      mVariableValuesX[iNode][jNode] =
          valueIJ < mInputParameters->UserCut.EpsForIntegrality ? 0.0 : valueIJ;

      double const valueJI = getNodeRel(mVariablesX[jNode][iNode]);
      mVariableValuesX[jNode][iNode] =
          valueJI < mInputParameters->UserCut.EpsForIntegrality ? 0.0 : valueJI;
    }
  }
}
...
void SubtourCallback::CheckIntegerSolution() {
  ...
  auto subtourTime = measure<>::duration(
      std::bind_front(&SubtourCallback::FindIntegerSubtours, this));
  CallbackTracker.UpdateElement(CallbackElement::DetermineRoutes,
                                static_cast<uint64_t>(subtourTime.count()));

  // Check integer routes for feasibility
  auto [solutionFeasible, checkRoutesTime] = measure<>::durationWithReturn(
      std::bind_front(&SubtourCallback::CheckRoutes, this));
  CallbackTracker.UpdateElement(CallbackElement::CheckRoutes,
                                static_cast<uint64_t>(checkRoutesTime.count()));
  ...
  CallbackTracker.UpdateUpperBound(this->getDoubleInfo(GRB_CB_RUNTIME),
                                   newCosts, mSolSPheuristic);
...
void SubtourCallback::FindIntegerSubtours() {
  ...
  for (const auto& node : mInstance->Nodes) {
    if (mVariableValuesX[mInstance->GetDepotId()][node.InternId] >
        integerThreshold) {
  ...
  while (counterVisitedNodes < mInstance->Nodes.size() - 1) {
    ...
    mSubtours.emplace_back(connectedToDepot, customerSequence, customersInRoute,
...
bool SubtourCallback::RouteCheckedAndFeasible(
  ...
  if (mLoadingChecker->RouteIsInFeasSequences(sequence)) {
    mClock.end();
    CallbackTracker.UpdateElement(CallbackElement::RoutePrechecked,
                                  mClock.elapsed());
  ...
  mClock.end();
  CallbackTracker.UpdateElement(CallbackElement::RoutePrecheckedNot,
                                mClock.elapsed());

  return false;
}

bool SubtourCallback::CustomerCombinationInfeasible(
    const Collections::IdVector& sequence,
  ...
  if (mLoadingChecker->CustomerCombinationInfeasible(combination)) {
    AddLazyConstraints({mLazyConstraintsGenerator->CreateConstraint(
        CutType::SEC, sequence, 2)});
    mLoadingChecker->AddInfeasibleCombination(combination);
    mClock.end();
    CallbackTracker.UpdateElement(CallbackElement::CustCombiInf,
                                  mClock.elapsed());
  ...
  mClock.end();
  CallbackTracker.UpdateElement(CallbackElement::CustCombiInfNot,
                                mClock.elapsed());

  return false;
}

void SubtourCallback::AddLazyConstraints(
    const std::vector<Cut>& lazyConstraints) {
...
void SubtourCallback::CheckFractionalSolution() {
  ...
  mCurrentNode =
      static_cast<unsigned int>(this->getDoubleInfo(GRB_CB_MIPNODE_NODCNT));
  ...
  if (mCurrentNode > mInputParameters->BranchAndCut.CutSeparationStartNodes &&
      (mCurrentNode - mLastNodeCount) <
          mInputParameters->BranchAndCut.CutSeparationThreshold) {
    return;
  }

  auto addFracCutTime =
      measure<>::duration(
          std::bind_front(&SubtourCallback::AddFractionalCuts, this))
          .count();
  CallbackTracker.UpdateElement(CallbackElement::AddFracSolCuts,
                                static_cast<uint64_t>(addFracCutTime));

  /*
  mLogFile << "VRPSEP " <<
  CallbackTracker.Counter[CallbackElement::FractionalSolutions] << ";"
           << mCurrentNode
...
void SubtourCallback::AddFractionalCuts() {
  ...
  mCVRPSEPGraph.Build(mVariableValuesX,
                      mInputParameters->UserCut.EpsForIntegrality);
  mClock.end();
  ...
  for (const auto& cut : mCutTypesFractional) {
    ...
    ////std::cout << "current node: " << currentNode << " | " << "Start
    /// Separation " << (int)cut_ptr->Type << "\n";
...
void SubtourCallback::AddCuts(const std::vector<Cut>& cuts) {
  size_t maxCutsToAdd =
      std::min(static_cast<size_t>(
                   mInputParameters->UserCut.MaxCutsAdd.at(cuts[0].Type)),
               cuts.size());
...
bool SubtourCallback::SolveSetPartitioningHeuristic() {
  ...
  if (mLoadingChecker->GetNumberOfFeasibleRoutes() <
  ...
  mLogFile << mCurrentNode << " : SP-Heuristic with "
           << mLoadingChecker->GetNumberOfFeasibleRoutes() << " routes."
           << "\n";

  auto newSolution =
      SPHeuristic->Run(this->getDoubleInfo(GRB_CB_MIPNODE_OBJBST) - 1e-5);

  mLogFile << "SC ObjVal:" << SPHeuristic->GetSCObjVal()
           << " | SP ObjVal: " << SPHeuristic->GetSPObjVal() << "\n";
...
void SubtourCallback::SetHeuristicSolution(
    const Collections::SequenceVector& routes) {
  ...
  std::cout << "Solution found with heuristic with " << routes.size()
            << " vehicles and costs " << std::to_string(objVal) << "\n";
}

void SubtourCallback::InjectSolution() {
  throw std::runtime_error("Not implemented.");
  // NOLINTBEGIN(readability-magic-numbers)
  Collections::SequenceVector solution{
      {1, 22, 28},  {4, 2},           {6, 21, 36}, {13, 19, 14},
      {20, 37, 5},  {25, 18, 24, 23}, {26, 11},    {27, 15, 29, 30},
      {31, 39, 12}, {32, 9},          {33, 16, 3}, {34, 8, 35, 7},
      {38, 10},     {40, 17}};
...
bool SubtourCallback1D::CheckRoutes() {
  ...
  for (const auto& subtour : mSubtours) {
    ...
    mClock.start();
    int minVehicles = mLoadingChecker->DetermineMinVehicles(
        mInputParameters->BranchAndCut.EnableMinVehicleLifting,
        mInputParameters->BranchAndCut.MinVehicleLiftingThreshold, container,
        subtour.CustomersInRoute, subtour.TotalWeight, subtour.TotalVolume);
    mClock.end();
...
bool SubtourCallback3D::CheckRoutes() {
  ...
  for (const auto& subtour : mSubtours) {
    ...
    if (minVehicles == 1) {
      auto checkFunc =
          std::bind_front(&SubtourCallback3D::CheckSingleVehicleSubtour, this,
                          subtour, container);
      auto [routeStatus, singleVehicleTime] =
          measure<>::durationWithReturn(checkFunc);
      CallbackTracker.UpdateElement(
          CallbackElement::SingleVehicle,
...
LoadingStatus SubtourCallback3D::CheckSingleVehicleSubtour(
    const Subtour& subtour, Container& container) {
  if (RouteCheckedAndFeasible(subtour.Sequence)) {
    return LoadingStatus::FeasOpt;
  }

  if (CustomerCombinationInfeasible(subtour.Sequence,
                                    subtour.CustomersInRoute)) {
    return LoadingStatus::Infeasible;
  }

  auto selectedItems = InterfaceConversions::SelectItems(
      subtour.Sequence, mInstance->Nodes, false);
...
bool SubtourCallback3D::CheckRouteHeuristic(
    const Collections::IdVector& sequence, Container& container,
  ...
  mClock.start();
  auto status = mLoadingChecker->PackingHeuristic(PackingType::Complete,
                                                  container, sequence, items);

  if (status == LoadingStatus::FeasOpt) {
    LocalSearch::RunIntraImprovement(mInstance, mLoadingChecker,
                                     mInputParameters, sequence);
    mClock.end();
    CallbackTracker.UpdateElement(CallbackElement::HeuristicFeas,
                                  mClock.elapsed());
    return true;
  }

  mClock.end();
  CallbackTracker.UpdateElement(CallbackElement::HeuristicInf,
                                mClock.elapsed());
...
void SubtourCallback3D::CheckReversePath(const Collections::IdVector& sequence,
  ...
  std::reverse_copy(std::begin(sequence), std::end(sequence),
                    std::begin(reversedSequence));

  if (mLoadingChecker->RouteIsInFeasSequences(reversedSequence)) {
    return;
  }

  auto items = InterfaceConversions::SelectItems(reversedSequence,
                                                 mInstance->Nodes, false);

  FunctionTimer timer;
  timer.start();

  auto heurStatus = mLoadingChecker->PackingHeuristic(
      PackingType::Complete, container, reversedSequence, items);

  if (heurStatus == LoadingStatus::FeasOpt) {
    LocalSearch::RunIntraImprovement(mInstance, mLoadingChecker,
                                     mInputParameters, reversedSequence);
    timer.end();
    CallbackTracker.UpdateElement(CallbackElement::RevHeurFeas,
                                  timer.elapsed());
  ...
  switch (cpStatus) {
    case LoadingStatus::FeasOpt: {
      LocalSearch::RunIntraImprovement(mInstance, mLoadingChecker,
                                       mInputParameters, reversedSequence);
      timer.end();
      CallbackTracker.UpdateElement(CallbackElement::RevExactFeas,
                                    timer.elapsed());
    ...
    case LoadingStatus::Infeasible: {
      ...
      timer.end();
      CallbackTracker.UpdateElement(CallbackElement::RevExactInf,
                                    timer.elapsed());
...
LoadingStatus SubtourCallback3DAllSimple::CheckRouteExact(
    const Subtour& subtour, Container& container, std::vector<Cuboid>& items) {
  ...
  switch (exactStatus) {
    case LoadingStatus::FeasOpt:
      LocalSearch::RunIntraImprovement(mInstance, mLoadingChecker,
                                       mInputParameters, subtour.Sequence);
      mClock.end();
      CallbackTracker.UpdateElement(CallbackElement::ExactFeas,
                                    mClock.elapsed());

      return LoadingStatus::FeasOpt;
    case LoadingStatus::Infeasible:
      mClock.end();
      CallbackTracker.UpdateElement(CallbackElement::ExactInf,
                                    mClock.elapsed());

      break;
    case LoadingStatus::Invalid:
      mClock.end();
      CallbackTracker.UpdateElement(CallbackElement::ExactInvalid,
                                    mClock.elapsed());
      return LoadingStatus::Invalid;
    case LoadingStatus::Unknown:
      mClock.end();
      throw std::runtime_error(
          "LoadingStatus is Unknown after exact CP model in "
          "CheckRouteExact().");
  }

  mClock.start();
  AddLazyConstraints({mLazyConstraintsGenerator->CreateConstraint(
      CutType::InfeasibleTailPath, subtour.Sequence)});
  mClock.end();
  CallbackTracker.UpdateElement(CallbackElement::InfeasibleTailPathInequality,
                                mClock.elapsed());

  return LoadingStatus::Infeasible;
}

bool SubtourCallback3DAll::Lifting(const Subtour& subtour, Container& container,
                                   std::vector<Cuboid>& items) {
  mClock.start();
  auto twoPathInequalities =
      mLazyConstraintsGenerator->TwoPathInequalityLifting(
          subtour.Sequence, subtour.CustomersInRoute, container, items);
  if (twoPathInequalities) {
    AddLazyConstraints(*twoPathInequalities);
    mClock.end();
    CallbackTracker.UpdateElement(CallbackElement::TwoPathInequality,
                                  mClock.elapsed());
  ...
  mClock.end();
  CallbackTracker.UpdateElement(CallbackElement::TwoPathInequalityNot,
                                mClock.elapsed());

  mClock.start();
  auto regularPathInequalities = mLazyConstraintsGenerator->RegularPathLifting(
      subtour.Sequence, container, items);
  if (regularPathInequalities) {
    AddLazyConstraints(*regularPathInequalities);
    mClock.end();
    CallbackTracker.UpdateElement(CallbackElement::RegularPathInequality,
                                  mClock.elapsed());
  ...
  mClock.end();
  CallbackTracker.UpdateElement(CallbackElement::RegularPathInequalityNot,
                                mClock.elapsed());

  return false;
}

LoadingStatus SubtourCallback3DAll::CheckRouteExact(
    const Subtour& subtour, Container& container, std::vector<Cuboid>& items) {
  // Solve complete CP model with time limit
  // Try lifting although sequence might be feasible (status unknown with time
  // limit) Reasoning: feasibility can be proven quickly -> mabye lifting with
  // relaxed problem is faster than solving complete problem
  mClock.start();
  ...
  switch (exactStatus) {
    case LoadingStatus::FeasOpt:
      LocalSearch::RunIntraImprovement(mInstance, mLoadingChecker,
                                       mInputParameters, subtour.Sequence);
      mClock.end();
      CallbackTracker.UpdateElement(CallbackElement::ExactLimitFeas,
                                    mClock.elapsed());

      return LoadingStatus::FeasOpt;
    case LoadingStatus::Unknown:
      mClock.end();
      CallbackTracker.UpdateElement(CallbackElement::ExactLimitUnk,
                                    mClock.elapsed());

      break;
    case LoadingStatus::Infeasible:
      mClock.end();
      CallbackTracker.UpdateElement(CallbackElement::ExactLimitInf,
                                    mClock.elapsed());
  ...
  if (exactStatus == LoadingStatus::Unknown) {
    double residualTime = mInputParameters->MIPSolver.TimeLimit -
                          this->getDoubleInfo(GRB_CB_RUNTIME);
    double maxRuntime = mInputParameters->DetermineMaxRuntime(
  ...
  mClock.start();
  AddLazyConstraints({mLazyConstraintsGenerator->CreateConstraint(
      CutType::TailTournament, subtour.Sequence)});
  mClock.end();
  ...
  mClock.end();
  CallbackTracker.UpdateElement(CallbackElement::ReverseSequence,
                                mClock.elapsed());

  return LoadingStatus::Infeasible;
}

void SubtourCallback3DAll::AddReversePathConstraints(
    const Collections::IdVector& sequence,
    const Collections::IdVector& reverseSequence) {
  AddLazyConstraints({mLazyConstraintsGenerator->CreateConstraint(
                          CutType::UndirectedTailPath, sequence),
                      mLazyConstraintsGenerator->CreateConstraint(
                          CutType::TailTournament, reverseSequence)});
}

bool SubtourCallback3DNoSupport::Lifting(const Subtour& subtour,
                                         Container& container,
                                         std::vector<Cuboid>& items) {
  mClock.start();
  auto twoPathInequalities =
...
LoadingStatus SubtourCallback3DNoSupport::CheckRouteExact(
    const Subtour& subtour, Container& container, std::vector<Cuboid>& items) {
  mClock.start();
  double maxRuntimeExactLimit = mInputParameters->DetermineMaxRuntime(
      BranchAndCutParams::CallType::ExactLimit);
  ...
  mClock.start();
  AddLazyConstraints(mLazyConstraintsGenerator->CreateRegularPathCuts(
      subtour.Sequence, container));
  mClock.end();
  CallbackTracker.UpdateElement(CallbackElement::RegularPathInequality,
                                mClock.elapsed());
...
void SubtourCallback3DNoSupport::AddReversePathConstraints(
    const Collections::IdVector& sequence,
    const Collections::IdVector& reverseSequence) {
  AddLazyConstraints({mLazyConstraintsGenerator->CreateConstraint(
                          CutType::UndirectedPath, sequence),
                      mLazyConstraintsGenerator->CreateConstraint(
                          CutType::RegularPath, reverseSequence)});
}

bool SubtourCallback3DNoLIFO::Lifting(const Subtour& subtour,
                                      Container& container,
                                      std::vector<Cuboid>& items) {
  mClock.start();
  auto twoPathInequalities =
...
LoadingStatus SubtourCallback3DNoLIFO::CheckRouteExact(
    const Subtour& subtour, Container& container, std::vector<Cuboid>& items) {
  ...
  switch (exactStatus) {
    case LoadingStatus::FeasOpt:
      LocalSearch::RunIntraImprovement(mInstance, mLoadingChecker,
                                       mInputParameters, subtour.Sequence);
      mClock.end();
      element = callType == Exact ? CallbackElement::ExactFeas
                                  : CallbackElement::ExactLimitFeas;
    ...
    case LoadingStatus::Infeasible:
      mClock.end();
      element = callType == Exact ? CallbackElement::ExactInf
                                  : CallbackElement::ExactLimitInf;
      CallbackTracker.UpdateElement(element, mClock.elapsed());
  ...
  AddLazyConstraints({mLazyConstraintsGenerator->CreateConstraint(
      CutType::TwoPathTail, subtour.Sequence)});

  return LoadingStatus::Infeasible;
}

void SubtourCallback3DNoLIFO::AddReversePathConstraints(
    const Collections::IdVector& sequence [[maybe_unused]],
...
bool SubtourCallback3DLoadingOnly::Lifting(const Subtour& subtour,
                                           ...
                                           [[maybe_unused]]) {
  mClock.start();
  auto twoPathInequalities = mLazyConstraintsGenerator->CreateTwoPathCuts(
      subtour.Sequence, subtour.CustomersInRoute, container);
  AddLazyConstraints(twoPathInequalities);
  mClock.end();
  CallbackTracker.UpdateElement(CallbackElement::TwoPathInequality,
                                mClock.elapsed());
...
LoadingStatus SubtourCallback3DLoadingOnly::CheckRouteExact(
    const Subtour& subtour, Container& container, std::vector<Cuboid>& items) {
  using enum BranchAndCutParams::CallType;
...
void SubtourCallback3DLoadingOnly::AddReversePathConstraints(
    const Collections::IdVector& sequence [[maybe_unused]],
    const Collections::IdVector& reverseSequence [[maybe_unused]]) {
  // No cut must be added.
}

std::unique_ptr<SubtourCallback> CallbackFactory::CreateCallback(
    LoadingProblemParams::VariantType variant, GRBEnv* env, GRBVar2D& vars,
    const Instance* const instance, LoadingChecker* loadingChecker,
    const InputParameters* const inputParameters, std::string& outputPath) {
  ...
  switch (variant) {
    ...
    case LoadingProblemParams::VariantType::VolumeWeightApproximation:
      callback = std::make_unique<SubtourCallback1D>(
    ...
    case LoadingProblemParams::VariantType::NoLifo:
      callback = std::make_unique<SubtourCallback3DNoLIFO>(
          vars, instance, loadingChecker, inputParameters, outputPath);
      break;
    ...
    case LoadingProblemParams::VariantType::AllConstraints:
      callback = std::make_unique<SubtourCallback3DAll>(
          vars, instance, loadingChecker, inputParameters, outputPath);
      break;
    default:
  ...
  if (inputParameters->BranchAndCut.SimpleVersion) {
    if (variant != LoadingProblemParams::VariantType::AllConstraints) {
      throw std::runtime_error(
          "Simple branch-and-cut version only for the all constraint variant "
          "implemented.");
    }

    callback = std::make_unique<SubtourCallback3DAllSimple>(
        vars, instance, loadingChecker, inputParameters, outputPath);
  }

  if (inputParameters->BranchAndCut.ActivateSetPartitioningHeuristic) {
    callback->SPHeuristic = std::make_unique<Heuristics::SetBased::SPHeuristic>(
        instance, loadingChecker, inputParameters, env);
  }
