...
void TwoIndexVehicleFlow::BuildModel(
    const std::vector<Arc>& startSolutionArcs,
...
void TwoIndexVehicleFlow::SetCallback(GRBCallback* callback) {
    ...
    while (totalNodesVisited < mInstance->Nodes.size() - 1) {
      ...
      while (nextNode != startNode) {
        for (const auto& node : mInstance->Nodes) {
          ...
          if (!nodeVisited[id] && mVariablesX[currentNodeId][id].get(
                                      GRB_DoubleAttr_X) > integerThreshold) {
            if (id != 0) {
              nodeVisited.set(id);
      ...
      solution.Tours.emplace_back(mInstance->Nodes[0], vehicle,
                                  std::move(route));
...
void TwoIndexVehicleFlow::SetStartSolution(
    const std::vector<Arc>& startSolutionArcs) {
...
void TwoIndexVehicleFlow::SetInfeasibleArcs(
    const std::vector<Arc>& infeasibleArcs,
    const std::vector<Arc>& infeasibleTailPaths) {
  for (auto const& arc : infeasibleArcs) {
    mVariablesX[arc.Tail][arc.Head].set(GRB_DoubleAttr_UB, arc.Coefficient);

    ////std::cout << "Infeasible arc: " << std::to_string(arc.Head) << " -> " <<
    /// std::to_string(arc.Tail) << "\n";
  }

  for (auto const& arc : infeasibleTailPaths) {
    mModel->addConstr(mVariablesX[arc.Tail][arc.Head] +
                          mVariablesX[arc.Head][mInstance->GetDepotId()] <=
                      1.0);

    ////std::cout << "Infeasible path: " << std::to_string(arc.Head) << " -> "
    ///<< std::to_string(arc.Tail) << "\n";
  }
}
...
void TwoIndexVehicleFlow::AddVariables() {
  for (const auto& nodeI : mInstance->Nodes) {
    ...
    for (const auto& nodeJ : mInstance->Nodes) {
      std::string name = "x_" + std::to_string(nodeI.InternId) + "_" +
                         std::to_string(nodeJ.InternId);
      mVariablesX.back().emplace_back(
          mModel->addVar(0, 1, 0, GRB_BINARY, name));
    }
  }
}
...
void TwoIndexVehicleFlow::AddConstraints() {
  for (const auto& nodeI : mInstance->GetCustomers()) {
    ...
    mModel->addConstr(sumXOut, GRB_EQUAL, 1.0,
                      "Outdegree_" + std::to_string(nodeI.InternId));
    mModel->addConstr(sumXIn, GRB_EQUAL, 1.0,
                      "Indegree_" + std::to_string(nodeI.InternId));
  }
  ...
  mModel->addConstr(sumXOutDepot, GRB_LESS_EQUAL,
                    static_cast<double>(mInstance->Vehicles.size()),
                    "Max_Outdegree_depot");
  mModel->addConstr(sumXOutDepot, GRB_GREATER_EQUAL,
                    static_cast<double>(mInstance->LowerBoundVehicles),
                    "Min_Outdegree_depot");
...
void TwoIndexVehicleFlow::AddObjective() {
  ...
  for (const auto& nodeI : mInstance->Nodes) {
    for (const auto& nodeJ : mInstance->Nodes) {
      totalDistance += mInstance->Distance(nodeI.InternId, nodeJ.InternId) *
                       mVariablesX[nodeI.InternId][nodeJ.InternId];
    }
  }
